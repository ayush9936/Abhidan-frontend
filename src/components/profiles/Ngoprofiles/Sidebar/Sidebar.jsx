...
import {BsFillCheckSquareFill,BsFillExclamationOctagonFill,BsFillCaretLeftSquareFill} from "react-icons/bs";
      ...
      <div className='side-bar' style={{width:open ? "250px" : "50px "}}>
        ...
        {   
          sideMenu.map((item,index)=>(
            <NavLink key={index}  className="sidebar-items border-2  " onClick ={()=> setPage(item.page)}>
              <div className='icons'>{item.icon}</div>
              <div className='side_text  text-lg font-semibold'style={{display: open ? "block" : "none"}} >{item.name}</div>
            </NavLink>
          ))
        }
